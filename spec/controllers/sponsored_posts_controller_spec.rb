...
RSpec.describe SponsoredPostsController, type: :controller do
  ...
  let(:my_ad) { my_topic.sponsored_posts.create!(title: RandomData.random_sentence, body: RandomData.random_paragraph, price: 100) }
 ...
 describe "GET show" do
     ...
     it "assigns my_ad to @post" do
       get :show, topic_id: my_topic.id, id: my_ad.id
       expect(assigns(:sponsored_post)).to eq(my_ad)
     end
   end
 ...
 # #2
   ...
   describe "PUT update" do
     it "updates post with expected attributes" do
       ...
       put :update, topic_id: my_topic.id, id: my_ad.id, sponsored_post: {title: new_title, body: new_body}
 
 # #3
       updated_post = assigns(:sponsored_post)
       expect(updated_post.id).to eq my_ad.id
     ...
     it "redirects to the updated post" do
       ...
       put :update, topic_id: my_topic.id, id: my_ad.id, sponsored_post: {title: new_title, body: new_body, price: 100}
   ...
   describe "DELETE destroy" do
     it "deletes the post" do
       ...
       count = SponsoredPost.where({id: my_ad.id}).size
